...
class Shortener(http.server.BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        if "longuri" not in params or "shortname" not in params:
            ...
            self.send_header('content-type', 'text/plain; charset=utf-8')
            self.end_headers()
            self.wfile.write('Not all fields in the form are filled in'.encode())
            return
        ...
        if CheckURI(longuri):
            ...
            self.send_header('content-type', 'text/html')
            self.end_headers()
            self.wfile.write('<h1>Long URI seems invalid!</h1>'.encode())
...
if __name__ == '__main__':
    port = int(os.environ.get('PORT', 8000)) # Use PORT if given
